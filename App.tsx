...
const App: React.FC = () => {
  ...
  const handleError = (err: any) => {
    ...
    if (msg.includes("401") || msg.includes("Requested entity was not found") || msg.includes("API_KEY_INVALID")) {
      setError("API Key tidak valid. Masukkan ulang.");
      handleLogout();
    } else {
      setError(msg);
    }
  ...
  const startProcessing = async () => {
    ...
    setLoadingMsg("Menyatukan Subjek & Busana...");
    startProgress();
    try {
  ...
  const handleRefine = async () => {
    ...
    setLoadingMsg("Menata Ruang Studio...");
    startProgress();
    try {
  ...
  const goToStoryboard = async () => {
    if (!state.combinedImage) return;
    setLoadingMsg("Merancang Storyboard 9-Angle...");
    startProgress();
    try {
  ...
  if (!isActivated) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#080809] p-6">
        <div className="max-w-md w-full glass p-10 sm:p-14 rounded-[3rem] border border-blue-500/20 text-center animate-up relative z-10">
          <div className="w-20 h-20 bg-gradient-to-tr from-blue-600 to-cyan-400 rounded-3xl mx-auto flex items-center justify-center mb-8 rotate-6">
            <i className="fa-solid fa-key text-3xl text-white"></i>
          </div>
          <h1 className="text-3xl font-black mb-3 tracking-tighter uppercase">Aktivasi Alat</h1>
          <p className="text-zinc-500 text-sm mb-10">Masukkan API Key Gemini untuk mulai.</p>
          <input type="password" placeholder="AIzaSy..." value={apiKeyInput} onChange={(e) => setApiKeyInput(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-2xl py-5 px-6 mb-6 outline-none focus:ring-2 focus:ring-blue-500/50" />
          {error && <p className="text-red-400 text-[10px] font-black uppercase mb-4">{error}</p>}
          <button onClick={handleActivate} className="w-full bg-blue-600 hover:bg-blue-500 py-6 rounded-2xl font-black transition-all uppercase tracking-widest text-xs">AKTIFKAN</button>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-[1400px] mx-auto pb-32">
          ...
          {step === AppStep.UPLOAD && (
            <div className="grid lg:grid-cols-2 gap-8 animate-up">
              {[ {t: 'Subject Model', d: 'model'}, {t: 'Product Asset', d: 'product'} ].map((u) => (
                <div key={u.d} className="glass p-10 rounded-[2.5rem] flex flex-col items-center text-center">
                  ...
                  <label className="w-full h-96 border-2 border-dashed border-zinc-800 hover:border-blue-500/50 rounded-[2rem] flex flex-col items-center justify-center cursor-pointer overflow-hidden relative group">
                    {(state as any)[`${u.d}Image`] ? <img src={(state as any)[`${u.d}Image`]} className="w-full h-full object-contain p-6" alt={u.t} /> : <div className="flex flex-col items-center opacity-40"><i className="fa-solid fa-plus text-3xl mb-4"></i><span className="text-[10px] font-black uppercase">Klik Pilih File</span></div>}
              ...
              <div className="lg:col-span-2 flex justify-center pt-8">
                <button disabled={!state.modelImage || !state.productImage} onClick={startProcessing} className="bg-blue-600 hover:bg-blue-500 disabled:bg-zinc-800 px-20 py-7 rounded-3xl font-black uppercase tracking-[0.3em] shadow-2xl">Mulai Produksi</button>
              </div>
            </div>
          )}
            ...
            <div className="grid lg:grid-cols-2 gap-12 animate-up">
              ...
              <div className="space-y-10">
                ...
                <div className="glass p-8 rounded-[2.5rem] space-y-6">
                  <div><label className="block text-[10px] font-black text-zinc-500 uppercase mb-3">Prompt Lingkungan</label><textarea value={options.background} onChange={(e) => setOptions(o => ({...o, background: e.target.value}))} className="w-full bg-black/40 border border-white/5 rounded-2xl p-5 text-xs h-32 resize-none" /></div>
                  <div><label className="block text-[10px] font-black text-blue-500 uppercase mb-3">Neon Sign Text</label><input type="text" value={options.neonText} onChange={(e) => setOptions(o => ({...o, neonText: e.target.value}))} className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-4 text-sm font-black uppercase" /></div>
                </div>
                <div className="flex flex-col gap-4">
                  <button onClick={handleRefine} className="py-5 rounded-2xl border border-white/10 hover:bg-white/5 font-bold text-[10px] uppercase">Update Preview</button>
                  <button onClick={goToStoryboard} className="py-7 rounded-[2rem] bg-blue-600 hover:bg-blue-500 font-black uppercase tracking-[0.3em] shadow-xl">Lanjut Storyboard</button>
                </div>
              </div>
            </div>
          )}
          ...
          {step === AppStep.STORYBOARD && state.storyboardGrid && (
            <div className="max-w-3xl mx-auto flex flex-col items-center animate-up">
              ...
              <div className="glass p-5 rounded-[3rem] mb-12"><img src={state.storyboardGrid} className="w-full rounded-[2.5rem] aspect-[9/16] object-contain shadow-2xl" /></div>
          ...
          {step === AppStep.RESULTS && (
            <div className="space-y-12 animate-up">
              <div className="flex justify-between items-end px-4">
                <div><h2 className="text-3xl font-black tracking-tighter mb-2">Final Output Assets</h2><p className="text-[10px] font-black text-zinc-500 uppercase">9 Scenes Diekstrak</p></div>
                <div className="text-right">
                  <p className="text-[9px] font-black text-blue-500 uppercase mb-2">Extraction Progress</p>
                    ...
                    <div key={scene.id} className="glass p-5 rounded-[2.5rem] flex flex-col h-full group">
                      ...
                      {scene.image && !scene.videoUrl && (
                        <div className="mt-auto space-y-3">
                          <textarea placeholder="Gerakan: Cinematic flow..." value={scenePrompts[idx]} onChange={(e) => { const n = [...scenePrompts]; n[idx] = e.target.value; setScenePrompts(n); }} className="w-full bg-black/20 border border-white/5 rounded-xl p-3 text-[10px] h-16 resize-none outline-none font-medium" />
